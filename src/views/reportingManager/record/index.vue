...
const getDate = async () => {
  recordList.value = await db.all(
     `select id,instanceID,name,age,recoredCreateTime,recoredTotalTime,recoredEndTime,describe,waveLength,eegInputMarkerList,irInputMarkerList,markerList from reportRecord where  '${searchValue.value}' = '' or name Like '%${searchValue.value}%'  or instanceID Like '%${searchValue.value}%'`
  );
};
